...
public class GuiTextBox extends Gui {
	...
	public void drawTextBox() {
		...
		for (int l = 0; l < lines; ++l) {
			...
			if (isFocused && selectEndIndex >= i && selectEndIndex <= j
				...
				Gui.drawRect(cursorX, y - 1, cursorX + 1, y + lineHeight,
						cursorColor);
			}
	...
	public boolean textboxKeyTyped(char typedChar, int keyCode) {
		...
		} else if (keyCode == Keyboard.KEY_X && GuiScreen.isCtrlKeyDown()) {
			action = 2;
			GuiScreen.setClipboardString(getSelectedText());
		} else if (keyCode == Keyboard.KEY_BACK) {
			action = 2;
			if (selectStartIndex == selectEndIndex) {
				moveSelectEndIndex(GuiScreen.isCtrlKeyDown(), false);
			}
		} else if (keyCode == Keyboard.KEY_DELETE) {
			action = 2;
			if (selectStartIndex == selectEndIndex) {
				moveSelectEndIndex(GuiScreen.isCtrlKeyDown(), true);
			}
		} else if (keyCode == Keyboard.KEY_HOME) {
			action = 1;
			selectEndIndex = GuiScreen.isCtrlKeyDown() ? 0
	...
	private void drawSelectionRect(int x1, int y1, int x2, int y2) {
		if (x1 < x2) {
			int x = x1;
			x1 = x2;
			x2 = x;
	 ...
	 * 
	 * @param lines
	 *            Number of lines to move; negative to move up.
	 */
	 ...
	 * @param pixels
	 *            Number of pixels to move. Set negative to move left.
	 * @param cap
	 *            Force return value to be between index and cap.
